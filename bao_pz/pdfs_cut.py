...
def pdfs_peaks_criteria(data,redshift_name,pdf_name,num_of_peaks):
	'''
	Find pdfs with more peaks that are less than 80% of the main peak
	data: fits table
	redshift_name: redshift columns name
	pdf_name: pdf column name
	num_of_peaks: number of peaks in a pdf
	'''
	peaks=[find_peaks(data[pdf_name][i],height=max(data[pdf_name][i])*.8,)[0] for i in range(len(data))]
